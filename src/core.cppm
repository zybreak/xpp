...
export namespace xpp {
    ...
    class core {
      protected:
        ...
        dispatch(std::string const &producer, xcb_generic_event_t *event) const {
            if (event) {
      ...
      public:
        ...
        virtual std::shared_ptr<xcb_generic_error_t>
        ...
        virtual xcb_query_extension_reply_t const *
        ...
        virtual std::tuple<std::string, int, int>
        parse_display(std::string const &name) const {
            int screen = 0;
            int display = 0;
            char *host = NULL;
            std::string hostname;

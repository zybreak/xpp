...
namespace xpp {
    ...
    public:
        connection_error(uint8_t code, const std::string &description) : std::runtime_error(description + "(" + std::to_string(code) + ")"), m_code(code), m_description(description) {
        }
    ...
    protected:
        ...
        shared_generic_event_ptr dispatch(const std::string &producer, xcb_generic_event_t *event) const {
            if (event) {
    ...
    public:
        explicit core(xcb_connection_t *c) : m_c(std::shared_ptr<xcb_connection_t>(c, [](...) {})) {
        }
        ...
        explicit core(const std::string &displayname = "") : core(xcb_connect, displayname.c_str(), &m_screen) {
        }

        // xcb_connect_to_fd (int fd, xcb_auth_info_t *auth_info)
        explicit core(int fd, xcb_auth_info_t *auth_info) : core(xcb_connect_to_fd, fd, auth_info) {
        }

        // xcb_connect_to_display_with_auth_info (
        //     const char *display, xcb_auth_info_t *auth, int *screen)
        explicit core(const std::string &display, xcb_auth_info_t *auth) : core(xcb_connect_to_display_with_auth_info,
        ...
        virtual shared_generic_event_ptr wait_for_special_event(xcb_special_event_t *se) const {
            return dispatch("wait_for_special_event", xcb_wait_for_special_event(m_c.get(), se));
        }
        ...
        virtual xcb_special_event_t *
        register_for_special_xge(xcb_extension_t *ext, uint32_t eid, uint32_t *stamp) const {
        ...
        virtual std::shared_ptr<xcb_generic_error_t> request_check(xcb_void_cookie_t cookie) const {
            return std::shared_ptr<xcb_generic_error_t>(xcb_request_check(m_c.get(), cookie));
        }
        ...
        virtual const xcb_query_extension_reply_t *get_extension_data(xcb_extension_t *ext) const {
        ...
        virtual const xcb_setup_t *get_setup() const {
        ...
        virtual std::tuple<std::string, int, int> parse_display(const std::string &name) const {
            int screen = 0;
            int display = 0;
            char *host = nullptr;
            std::string hostname;
        ...
        void check_connection() const {
            switch (xcb_connection_has_error(m_c.get())) {
                case XCB_CONN_ERROR:
                    throw (connection_error(XCB_CONN_ERROR, "XCB_CONN_ERROR"));

                case XCB_CONN_CLOSED_EXT_NOTSUPPORTED:
                    throw (connection_error(XCB_CONN_CLOSED_EXT_NOTSUPPORTED, "XCB_CONN_CLOSED_EXT_NOTSUPPORTED"));

                case XCB_CONN_CLOSED_MEM_INSUFFICIENT:
                    throw (connection_error(XCB_CONN_CLOSED_MEM_INSUFFICIENT, "XCB_CONN_CLOSED_MEM_INSUFFICIENT"));

                case XCB_CONN_CLOSED_REQ_LEN_EXCEED:
                    throw (connection_error(XCB_CONN_CLOSED_REQ_LEN_EXCEED, "XCB_CONN_CLOSED_REQ_LEN_EXCEED"));

                case XCB_CONN_CLOSED_PARSE_ERR:
                    throw (connection_error(XCB_CONN_CLOSED_PARSE_ERR, "XCB_CONN_CLOSED_PARSE_ERR"));

                case XCB_CONN_CLOSED_INVALID_SCREEN:
                    throw (connection_error(XCB_CONN_CLOSED_INVALID_SCREEN, "XCB_CONN_CLOSED_INVALID_SCREEN"));
            };
        }
    }; // class core

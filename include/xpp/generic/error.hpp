...
namespace xpp::generic {
    ...
    public:
        virtual void operator()(const std::shared_ptr<xcb_generic_error_t> &) const = 0;
    ...
    namespace detail {
        ...
        void dispatch(const Object &object, const std::shared_ptr<xcb_generic_error_t> &error, std::true_type) {
            static_cast<const xpp::generic::error_dispatcher &>(object)(error);
        }

        template<typename Object>
        void dispatch(const Object &, const std::shared_ptr<xcb_generic_error_t> &error, std::false_type) {
            throw error;
        }
    ...
    template<typename Object>
    void dispatch(const Object &object, const std::shared_ptr<xcb_generic_error_t> &error) {
        detail::dispatch(object, error, std::is_base_of<xpp::generic::error_dispatcher, Object>());
    }
    ...
    public:
        ...
        virtual operator const Error &() const {
            return reinterpret_cast<const Error &>(*m_error);
        }

        virtual const Error &operator*() const {
            return reinterpret_cast<const Error &>(*m_error);
        }

        virtual Error *operator->() const {
            return reinterpret_cast<Error *const>(m_error.get());
        }

    protected:
        virtual std::string get_error_description(xcb_generic_error_t *error) const {

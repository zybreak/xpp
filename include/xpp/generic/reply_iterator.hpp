...
namespace xpp::generic {
    ...
    class get<xcb_str_t> {
    public:
        std::string operator()(xcb_str_t *const data) {
            return std::string(xcb_str_name(data), xcb_str_name_length(data));
        }
    };
...
// iterator for variable size data fields
    ...
    template<typename Connection, typename Object, typename NextTemplate, NextTemplate &Next, typename SizeOfTemplate, SizeOfTemplate &SizeOf, typename GetIteratorTemplate, GetIteratorTemplate &GetIterator>
    class iterator<Connection, Object, xpp::generic::signature<NextTemplate, Next>, xpp::generic::signature<SizeOfTemplate, SizeOf>, xpp::generic::signature<GetIteratorTemplate, GetIterator>> {
    protected:
        using self = iterator<Connection, Object, xpp::generic::signature<NextTemplate, Next>, xpp::generic::signature<SizeOfTemplate, SizeOf>, xpp::generic::signature<GetIteratorTemplate, GetIterator>>;
    ...
    public:
        ...
        auto operator*() -> decltype(get<Object>()(this->m_iterator.data)) {
        ...
        template<typename C>
        static self begin(C &&c, const std::shared_ptr<Reply> &reply) {
            return self{std::forward<C>(c), reply};
        }

        template<typename C>
        static self end(C &&c, const std::shared_ptr<Reply> &reply) {
...
// iterator for fixed size data fields
    ...
    template<typename Connection, typename Object, typename AccessorTemplate, AccessorTemplate &Accessor, typename LengthTemplate, LengthTemplate &Length>
    class iterator<Connection, Object, signature<AccessorTemplate, Accessor>, signature<LengthTemplate, Length>> {
    protected:
        ...
        using data_t = typename std::conditional<std::is_void<Data>::value, typename xpp::generic::conversion_type<Object>::type, Data>::type;
    ...
    public:
        ...
        typedef iterator<Connection, Object, signature<AccessorTemplate, Accessor>, signature<LengthTemplate, Length>> self;

        iterator() {
        }

        template<typename C>
        iterator(C &&c, const std::shared_ptr<Reply> &reply, std::size_t index)
        ...
        template<typename C>
        static self end(C &&c, const std::shared_ptr<Reply> &reply) {
            return self{std::forward<C>(c), reply, static_cast<std::size_t>(Length(reply.get()))};
        }
    };

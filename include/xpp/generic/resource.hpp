...
namespace xpp::generic {
    ...
    namespace detail {
        ...
        template<typename Connection, typename Resource, typename ResourceId, template<typename, typename> class ... Interfaces>
        class interfaces : public Interfaces<interfaces<Connection, Resource, ResourceId, Interfaces ...>, Connection> ... {
        public:
    ...
    template<typename Connection, typename ResourceId, template<typename, typename> class ... Interfaces>
    class resource : public detail::interfaces<Connection, resource<Connection, ResourceId, Interfaces ...>, ResourceId, Interfaces ...> {
    protected:
        ...
        resource(Connection c) : m_c(c) {
        }

        template<typename C, typename Create, typename Destroy>
        static self make(C &&c, Create create, Destroy destroy) {
            ...
            resource.m_resource = std::shared_ptr<ResourceId>(new ResourceId(xid), [&](ResourceId *r) {
    ...
    public:
        ...
        resource(const resource<Connection, ResourceId, Interfaces ...> &other) : m_c(other.m_c), m_resource(other.m_resource) {
        }

        virtual void operator=(const resource<Connection, ResourceId, Interfaces ...> &other) {
            m_c = other.m_c;
            m_resource = other.m_resource;
        }

        virtual void operator=(const ResourceId &resource) {
            m_resource = std::make_shared<ResourceId>(resource);
        }

        virtual const ResourceId &operator*() const {
            return *m_resource;
        }

        virtual operator const ResourceId &() const {
            return *m_resource;
        }
    ...
    template<typename Connection, typename ResourceId, template<typename, typename> class ... Interfaces>
    std::ostream &operator<<(std::ostream &os, const resource<Connection, ResourceId, Interfaces ...> &resource) {
